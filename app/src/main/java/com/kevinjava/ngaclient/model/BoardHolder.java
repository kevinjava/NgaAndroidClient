...
public class BoardHolder {
	...
	/*
	 * public void convertChildren(){ Map<Integer,BoardCategory> newInfo = new
	 * HashMap<Integer,BoardCategory>(); for( Object key : boardInfo.keySet()){
	 * Object v = boardInfo.get(key); if( v instanceof JSONObject ) {
	 * BoardCategory b = JSON.toJavaObject((JSONObject)v, BoardCategory.class);
	 * //boardInfo.remove(key); b.convert(); Integer ki =
	 * Integer.parseInt((String)key); newInfo.put(ki, b); }else{
	...
	/*
	 * public void remove(String fid) { remove(0,fid);
	 * 
	 * }
	 */
