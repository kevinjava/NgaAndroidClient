...
public class StringUtil {
    ...
    private static final String lesserNukeStyle = "<div style='border:1px solid #B63F32;margin:10px 10px 10px 10px;padding:10px' > <span style='color:#EE8A9E'>NUKE</span><br/>";
    ...
    public static String decodeForumTag(String s, boolean showImage) {
        ...
        // reply
        s = s.replaceAll(ignoreCaseTag + "\\[pid=\\d+\\]Reply\\[/pid\\]",
                "Reply");
        ...
        s = s.replaceAll(ignoreCaseTag + "<br/><br/>", "<br/>");
        // [list] [/list] [*]
        s = s.replaceAll(ignoreCaseTag + "\\[list\\]", "");
        s = s.replaceAll(ignoreCaseTag + "\\[\\*\\]", " ");
        s = s.replaceAll(ignoreCaseTag + "\\[/list\\]", "");
        // [url][/url]
        s = s.replaceAll(
                ignoreCaseTag + "\\[url\\](http[^\\[|\\]]+)\\[/url\\]",
                "<a href=\"$1\">$1</a>");
        s = s.replaceAll(ignoreCaseTag
                        + "\\[url=([^\\[|\\]]+)\\]\\s*(.+?)\\s*\\[/url\\]",
                "<a href=\"$1\">$2</a>"
    ...
    // for testing
    public static String convertToHtmlText(final SubjectData data,final ReplyPageInfo row, boolean showImage, final String fgColorStr, final String bgcolorStr) {
...
//                + buildSignature(row.get__T(),showImage);
        ngaHtml = "<HTML> <HEAD><META   http-equiv=Content-Type   content= \"text/html;   charset=utf-8 \">"
                + buildHeader(data, row, fgColorStr)
    ...
    private static String buildComment(ReplyPageInfo row, String fgColor) {
        if (row == null || row.getComment() == null || row.getComment().size() == 0) {
            return "";
        }
        ...
        Iterator<Comment> it = row.getComment().iterator();
        while (it.hasNext()) {
            ...
            ret.append("</span><br/>");
            ret.append("<img src='");
            String avatarUrl = parseAvatarUrl("用户头像");
    ...
    private static String parseAvatarUrl(String js_escap_avatar) {
        ...
        try {
            ...
            NgaLog.e("TAG", "cann't handle avatar url " + js_escap_avatar);
        }
        return ret;
    }

    private static String buildAttachment(ReplyPageInfo row, boolean showImage) {
        if (row == null || row.getAttachs() == null || row.getAttachs().size() == 0) {
            return "";
        }
        StringBuilder ret = new StringBuilder();
        ret.append("<br/><br/>").append("附件").append("<hr/><br/>");
        ...
        ret.append("<tbody>");
        Iterator<AttachInfo> it = row.getAttachs().iterator();
        while (it.hasNext()) {
            AttachInfo info = it.next();
...
//    private static String buildSignature(SubjectData subData, boolean showImage){
//        ThreadPageInfo row = subData.get__T();
//        if(row ==null ){
//            return "";
//        }
...
//    }
    ...
    private static String buildHeader(SubjectData data, ReplyPageInfo row, String fgColorStr) {
        if (data == null || TextUtils.isEmpty(data.get__T().getSubject()) || row.getLou() != 0)
            return "";
        StringBuilder sb = new StringBuilder();
        sb.append("<h4 style='color:").append(fgColorStr).append("' >")
                .append(data.get__T().getSubject()).append("</h3>");
        return sb.toString();
    }
